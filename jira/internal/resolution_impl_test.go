...
func Test_internalResolutionImpl_Gets(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			resolutionService, err := NewResolutionService(testCase.fields.c, testCase.fields.version)
...
func Test_internalResolutionImpl_Get(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:   "when the resolution id is not provided",
			fields: fields{version: "3"},
			args: args{
				ctx: context.Background(),
			},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			gotResult, gotResponse, err := resolutionService.Get(testCase.args.ctx, testCase.args.resolutionID)
...
func Test_NewResolutionService(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			got, err := NewResolutionService(testCase.args.client, testCase.args.version)
